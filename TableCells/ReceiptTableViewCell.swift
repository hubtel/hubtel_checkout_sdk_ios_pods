...
class ReceiptTableViewCell: UITableViewCell {
    ...
    let topZigZag: UIImageView = {
        ...
        if #available(iOS 13.0, *) {
            view.image = imageNamed("receipt_zigzag")
        }else{
            view.image = imageNamed("receipt_zigzag")
        }
    ...
    lazy var bottomZigZag: UIImageView = {
        ...
        view.translatesAutoresizingMaskIntoConstraints = false
        if #available(iOS 13.0, *) {
            view.image = imageNamed("zigzagBottom").withRenderingMode(.alwaysOriginal).withTintColor((Colors.globalColor ?? tintColor)!.withAlphaComponent(0.2))
        }else{
            view.image = imageNamed("receipt_zigzag")
        }
